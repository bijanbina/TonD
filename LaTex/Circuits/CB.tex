...
\draw (Vcc) ++ (0,-\VccToMos) node[npn](mos) {};

%-------------Load & Out-------------
\coordinate (Node_out)  at  ($ (mos.collector) + (0,\OutputOffset) $);
...
\draw (mos.emitter) to[R=$R_S$] (GND) {};%GND
\draw (Vcc) to [R=\small{$R_D$}] (Node_out) -- (mos.collector);%RD
\draw (VccB) to [R=$R_1$] (GATE_IN) node(RG1) {};%RG1
\draw (RG1) to [R=$R_2$] (GNDB);%RG2
\draw (mos.base) -- (GATE_IN); %Short Circuit
%(mos.emitter) node[anchor=north] {S};
\nwTondComment{mos.collector}{$r_o$};
\swTondComment{mos.emitter}{$\frac{1}{g_m}$};

%----------------Signal--------------
\coordinate (C_IN) at  ($ (mos.emitter) + (\SmallElement,0) $);
\coordinate (R_IN) at  ($ (C_IN) +  (\SmallElement,0) $);
\coordinate (GND_SIG) at  ($ (R_IN)  - (0,2) $);

\draw (GND_SIG) node [ground] {};%GND
\draw (C_IN) to[C=$C$] (mos.emitter);%Rsig
